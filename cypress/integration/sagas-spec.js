...
describe('rootSaga()', () => {
  it('Yields effects', () => {
    ...
    expect(saga.next().value).to.deep.equal(yield1);
    expect(saga.next().value).to.deep.equal(yield2);
    expect(saga.next().value).to.deep.equal(yield3);
...
describe('createTodo()', () => {
  it('Handles success', () => {
    ...
    expect(saga.next().value).to.deep.equal(yield1);
    expect(saga.next(todo).value).to.deep.equal(yield2);
    expect(saga.next().done).to.equal(true);
  });
  ...
  it('Handles error', () => {
    ...
    expect(saga.next().value).to.deep.equal(yield1);
    expect(saga.throw(error).value).to.deep.equal(yield2);
    expect(saga.next().done).to.equal(true);
  });
});
...
describe('toggleTodoQueue()', () => {
  it('Handles action channel', () => {
    ...
    expect(saga.next(mockChannel).value).to.deep.equal(yield2);
    // Receive a toggleTodo action and pass it to the toggleTodo saga
    expect(saga.next(toggleTodoAction).value).to.deep.equal(yield3);
    // Receive a toggleTodoCancel action and flush the channel
    expect(saga.next({cancel: toggleTodoCancelAction}).value).to.deep.equal(
      yield4,
    );
